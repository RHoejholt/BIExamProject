# src/etl/map_utils.py  (replace file)
import numpy as np
import pandas as pd
...
def make_heatmap_overlay(
    ...
    - alpha_scale: multiplies alpha channel (1.0 = modest, >1 more opaque)
    - use_log: apply log(1+heat) before normalization to boost low densities
    - gamma: raise normalized intensity to power (gamma<1 brightens low-intensity)
    """
    if mm_df is None or mm_df.empty:
    ...
    # alpha (make peaks much more visible, adjustable by alpha_scale)
    # square-root then scale to emphasize lower values while still keeping peaks strong
    ...
    # Ensure overlay matches base size exactly
    try:
